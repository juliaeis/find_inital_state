...
def plot_experiment (solution,rgi_id,fls_num, plot_dir):
    ...
    plt.savefig(os.path.join(plot_dir,'experiment_HEF.png'),dpi=300)
    #plt.close()
    return
...
def plot_boxplot(volume,plot_dir):
    ...
    ax.set_xticklabels(['1880','2000'], fontsize=20)
...
def plot_histogramm(volume,plot_dir):
    ...
    ax.set_title(rgi_id + ': Hintereisferner', fontsize=25)
    ax.hist([volume['1880'],volume['2000']],bins=30,label=['1880','2000'])
        ...
        if file.endswith('00897_solution.pkl') :
                ...
                for res in results:
                    if res[0] != None:
                        ...
                        if i == len(solution[0].fls)-1:
                            volume = volume.append({'1880':res[0].volume_km3-solution[0].volume_km3,'2000':res[1].volume_km3-solution[1].volume_km3},ignore_index=True)
            ...
            for gdir in gdirs:
                if gdir.rgi_id =='RGI50-11.00897':
                    ...
                    fig, ax1 = plt.subplots(figsize=(15, 10))
                    cbar=graphics.plot_centerlines(gdir,ax=ax1)
                    ax1.tick_params(axis='both', which='major', labelsize=20)
